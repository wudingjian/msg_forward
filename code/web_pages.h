...
// CSS 样式 (独立路由 /style.css 提供，使用 PROGMEM 存储在 Flash)
...
// HTML 页面模板 (使用外部 CSS 引用)
const char htmlPage[] PROGMEM = R"rawhtml(<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=0"><title>短信转发器</title>
<link rel="stylesheet" href="/style.css">
<script>
const $=id=>document.getElementById(id);
const $s=sel=>document.querySelector(sel);
const postJ=(u,d,cb)=>{fetch(u,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(d)}).then(r=>r.json()).then(cb).catch(e=>toast('请求失败: '+e))};
let toastTimer;
...
<div class="nav">
  <div class="nav-item active" onclick="swTab(0)"><div class="nav-icon" style="-webkit-mask-image:url('data:image/svg+xml,%3Csvg viewBox=%220 0 24 24%22 xmlns=%22http://www.w3.org/2000/svg%22%3E%3Cpath d=%22M3 13h8V3H3v10zm0 8h8v-6H3v6zm10 0h8V11h-8v10zm0-18v6h8V3h-8z%22/%3E%3C/svg%3E')"></div><span>概览</span></div>
  <div class="nav-item" onclick="swTab(1)"><div class="nav-icon" style="-webkit-mask-image:url('data:image/svg+xml,%3Csvg viewBox=%220 0 24 24%22 xmlns=%22http://www.w3.org/2000/svg%22%3E%3Cpath d=%22M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 17.93c-3.95-.49-7-3.85-7-7.93 0-.62.08-1.21.21-1.79L9 15v1c0 1.1.9 2 2 2v1.93zm6.9-2.54c-.26-.81-1-1.39-1.9-1.39h-1v-3c0-.55-.45-1-1-1H8v-2h2c.55 0 1-.45 1-1V7h2c1.1 0 2-.9 2-2v-.41c2.93 1.19 5 4.06 5 7.41 0 2.08-.8 3.97-2.1 5.39z%22/%3E%3C/svg%3E')"></div><span>控制</span></div>
  <div class="nav-item" onclick="swTab(2)"><div class="nav-icon" style="-webkit-mask-image:url('data:image/svg+xml,%3Csvg viewBox=%220 0 24 24%22 xmlns=%22http://www.w3.org/2000/svg%22%3E%3Cpath d=%22M13 3c-4.97 0-9 4.03-9 9H1l3.89 3.89.07.14L9 12H6c0-3.87 3.13-7 7-7s7 3.13 7 7-3.13 7-7 7c-1.93 0-3.68-.79-4.94-2.06l-1.42 1.42C8.27 19.99 10.51 21 13 21c4.97 0 9-4.03 9-9s-4.03-9-9-9zm-1 5v5l4.28 2.54.72-1.21-3.5-2.08V8H12z%22/%3E%3C/svg%3E')"></div><span>历史</span></div>
  <div class="nav-item" onclick="swTab(3)"><div class="nav-icon" style="-webkit-mask-image:url('data:image/svg+xml,%3Csvg viewBox=%220 0 24 24%22 xmlns=%22http://www.w3.org/2000/svg%22%3E%3Cpath d=%22M12 15.5A3.5 3.5 0 0 1 8.5 12 3.5 3.5 0 0 1 12 8.5a3.5 3.5 0 0 1 3.5 3.5 3.5 3.5 0 0 1-3.5 3.5m7.43-2.53c.04-.32.07-.64.07-.97s-.03-.66-.07-1l2.11-1.63c.19-.15.24-.42.12-.64l-2-3.46c-.12-.22-.39-.31-.61-.22l-2.49 1c-.52-.39-1.06-.73-1.69-.98l-.37-2.65A.506.506 0 0 0 14 2h-4c-.25 0-.46.18-.5.42l-.37 2.65c-.63.25-1.17.59-1.69.98l-2.49-1c-.22-.09-.49 0-.61.22l-2 3.46c-.13.22-.07.49.12.64L4.57 11c-.04.34-.07.67-.07 1s.03.65.07.97l-2.11 1.66c-.19.15-.25.42-.12.64l2 3.46c.12.22.39.3.61.22l2.49-1.01c.52.4 1.06.74 1.69.99l.37 2.65c.04.24.25.42.5.42h4c.25 0 .46-.18.5-.42l.37-2.65c.63-.26 1.17-.59 1.69-.99l2.49 1.01c.22.08.49 0 .61-.22l2-3.46c.12-.22.07-.49-.12-.64l-2.11-1.66z%22/%3E%3C/svg%3E')"></div><span>配置</span></div>
</div>
...
<div class="page">
  ...
  <div class="card">
    ...
    <div class="sw-row"><span id="pStat">Ping 8.8.8.8 (会产生少量流量费用)</span> <button class="btn btn-w" style="width:auto;padding:6px 16px" onclick="if(confirm('确定要消耗流量保号吗？'))act('ping')">消耗流量</button></div>
...
<div class="page">
  ...
  <details open>
    ...
    <div class="det-body">
      ...
      <div style="font-size:0.85em;color:var(--text-light);margin-top:8px">💡 可配置多个WiFi，设备会自动连接信号最强的网络。修改后需重启生效。</div>
    </div>
  </details>
  <details>
      ...
      <div id="mqBox" style="display:%MQTT_DISP%">
         ...
         <div style="border-top:1px solid var(--border);margin:12px 0 8px;padding-top:12px">
           ...
           <div id="mqHaBox" style="display:%MQTT_HA_DISP%">
             ...
             <div style="font-size:0.85em;color:var(--text-light);margin-top:4px">💡 启用后设备将自动注册到 Home Assistant，无需手动配置 YAML 文件。</div>
           </div>
         </div>
      </div>
    </div>
        ...
        <div id="chb0" style="display:none;margin-top:12px;border-top:1px solid #f1f5f9;padding-top:12px">
          ...
          <div class="fg"><label>类型</label>
            <select name="push0type" id="tp0" onchange="upd(0)">
              <option value="1" %CH0_T1%>POST JSON</option><option value="2" %CH0_T2%>Bark</option><option value="3" %CH0_T3%>GET请求</option><option value="4" %CH0_T4%>自定义模板</option><option value="5" %CH0_T5%>Telegram Bot</option><option value="6" %CH0_T6%>企业微信</option><option value="7" %CH0_T7%>钉钉</option>
            </select>
          </div>
          ...
          <div id="cf0" style="display:%CH0_CFD%">
            <div class="fg"><label>Body 模板 (JSON)</label><textarea name="push0body" rows="4" placeholder='{"msg": "{message}", "from": "{sender}"}'>%CH0_BODY%</textarea><div style="font-size:0.75em;color:var(--text-light);margin-top:4px">支持变量: {sender}, {message}, {timestamp}, {device}</div></div>
          </div>
        </div>
      </div>
      <div class="card" style="border:1px solid #e2e8f0;padding:12px;margin-bottom:8px;box-shadow:none">
        ...
        <div id="chb1" style="display:none;margin-top:12px;border-top:1px solid #f1f5f9;padding-top:12px">
          ...
          <div class="fg"><label>类型</label>
            <select name="push1type" id="tp1" onchange="upd(1)">
              <option value="1" %CH1_T1%>POST JSON</option><option value="2" %CH1_T2%>Bark</option><option value="3" %CH1_T3%>GET请求</option><option value="4" %CH1_T4%>自定义模板</option><option value="5" %CH1_T5%>Telegram Bot</option><option value="6" %CH1_T6%>企业微信</option><option value="7" %CH1_T7%>钉钉</option>
            </select>
          </div>
          ...
          <div id="cf1" style="display:%CH1_CFD%">
            <div class="fg"><label>Body 模板 (JSON)</label><textarea name="push1body" rows="4" placeholder='{"msg": "{message}", "from": "{sender}"}'>%CH1_BODY%</textarea><div style="font-size:0.75em;color:var(--text-light);margin-top:4px">支持变量: {sender}, {message}, {timestamp}, {device}</div></div>
          </div>
        </div>
      </div>
      <div class="card" style="border:1px solid #e2e8f0;padding:12px;margin-bottom:8px;box-shadow:none">
        ...
        <div id="chb2" style="display:none;margin-top:12px;border-top:1px solid #f1f5f9;padding-top:12px">
          ...
          <div class="fg"><label>类型</label>
            <select name="push2type" id="tp2" onchange="upd(2)">
              <option value="1" %CH2_T1%>POST JSON</option><option value="2" %CH2_T2%>Bark</option><option value="3" %CH2_T3%>GET请求</option><option value="4" %CH2_T4%>自定义模板</option><option value="5" %CH2_T5%>Telegram Bot</option><option value="6" %CH2_T6%>企业微信</option><option value="7" %CH2_T7%>钉钉</option>
            </select>
          </div>
          ...
          <div id="cf2" style="display:%CH2_CFD%">
            <div class="fg"><label>Body 模板 (JSON)</label><textarea name="push2body" rows="4" placeholder='{"msg": "{message}", "from": "{sender}"}'>%CH2_BODY%</textarea><div style="font-size:0.75em;color:var(--text-light);margin-top:4px">支持变量: {sender}, {message}, {timestamp}, {device}</div></div>
          </div>
        </div>
      </div>
    </div>
...
// 配置页 HTML (Part 4: 过滤 + 定时任务)
const char HTML_PAGE_CONFIG4[] PROGMEM = R"rawhtml(
  <details>
    <summary>号码黑白名单过滤 <span id="ftModeBadge" class="badge b-wait" style="margin-left:auto;margin-right:8px">未启用</span></summary>
    <div class="det-body">
      ...
      <div id="ftModeBox" style="margin-top:12px">
        ...
        <div class="mode-toggle">
          <div class="mode-btn" id="modeBl" onclick="setFtMode(false)"><div class="mode-icon">🚫</div><div class="mode-title">黑名单模式</div><div class="mode-desc">拦截列表中的号码</div></div>
          <div class="mode-btn" id="modeWl" onclick="setFtMode(true)"><div class="mode-icon">✅</div><div class="mode-title">仅白名单</div><div class="mode-desc">只接收列表中的号码</div></div>
        </div>
        ...
        <div class="fg"><label id="ftListLabel">号码列表</label><textarea id="ftList" name="filterList" rows="5" placeholder="输入号码，多个号码用逗号分隔"></textarea></div>
      </div>
  ...
  <details>
    ...
    <div class="det-body">
      ...
      <div id="cfModeBox" style="margin-top:12px">
        <label style="margin-bottom:8px">过滤模式</label>
        <div class="mode-toggle">
          <div class="mode-btn" id="cfModeBl" onclick="setCfMode(false)"><div class="mode-icon">🚫</div><div class="mode-title">黑名单模式</div><div class="mode-desc">拦截包含关键词的短信</div></div>
          <div class="mode-btn" id="cfModeWl" onclick="setCfMode(true)"><div class="mode-icon">🔍</div><div class="mode-title">仅白名单</div><div class="mode-desc">只转发包含关键词的短信</div></div>
        </div>
        ...
        <div class="fg"><label id="cfListLabel">关键词列表</label><textarea id="cfList" name="contentFilterList" rows="4" placeholder="输入关键词，多个关键词用逗号分隔"></textarea></div>
        <div style="font-size:0.85em;color:var(--text-light);margin-bottom:12px">💡 关键词匹配不区分大小写。可用于过滤广告短信或只接收重要短信。</div>
      </div>
  ...
  <details>
    ...
    <div class="det-body">
      ...
      <div class="fg"><label>起始日期</label><input type="date" id="tmStart" name="timerStartDate" onchange="updTmInfo()"><div style="font-size:0.8em;color:var(--text-light);margin-top:4px">💡 设置起始日后，即使重启设备，倒计时也会根据起始日正确计算</div></div>
...
function updTmInfo(){
  ...
  var badge=$('tmBadge'),info=$('tmInfo'),typeName=tp===0?'Ping保活':'发送短信';
  if(!en){badge.className='badge b-wait';badge.innerText='未启用';info.innerHTML='<div style="color:#64748b">⏸ 定时任务已禁用</div>';return;}
  badge.className='badge b-ok';badge.innerText=typeName;
  var rmStr='未设置起始日',nextDateStr='-';
  if(startDate&&days>0){
  ...
  html+='<div><span style="color:#64748b">起始日期:</span> <b>'+(startDate||'未设置')+'</b></div>';
  html+='<div><span style="color:#64748b">下次执行:</span> <b>'+rmStr+'</b></div>';
  if(tp===1){html+='<div style="grid-column:span 2"><span style="color:#64748b">发送至:</span> <b>'+($('tmPh').value||'未设置')+'</b></div>';}
...
function updFtMode(){
  ...
  if(!en){badge.className='badge b-wait';badge.innerText='未启用';modeBox.style.opacity='0.5';}
  else{modeBox.style.opacity='1';badge.className='badge '+(wl?'b-ok':'b-err');badge.innerText=wl?'仅白名单':'黑名单';}
  if(wl){hint.innerHTML='<span style="color:#0369a1">✅ 仅白名单模式</span>：只有列表中的号码发送的短信会被转发，其他号码将被<b>拦截</b>。';$('ftListLabel').innerText='允许的号码 (只接收这些)';}
  else{hint.innerHTML='<span style="color:#b91c1c">🚫 黑名单模式</span>：列表中的号码发送的短信会被<b>拦截</b>，其他号码正常转发。';$('ftListLabel').innerText='拦截的号码 (屏蔽这些)';}
}
updFtMode();
...
function updCfMode(){
  ...
  if(!en){badge.className='badge b-wait';badge.innerText='未启用';modeBox.style.opacity='0.5';}
  else{modeBox.style.opacity='1';badge.className='badge '+(wl?'b-ok':'b-warn');badge.innerText=wl?'仅白名单':'黑名单';}
  if(wl){hint.innerHTML='<span style="color:#0369a1">🔍 仅白名单模式</span>：只有包含关键词的短信会被转发，其他短信将被<b>拦截</b>。';$('cfListLabel').innerText='必须包含的关键词 (只转发这些)';}
  else{hint.innerHTML='<span style="color:#a16207">🚫 黑名单模式</span>：包含关键词的短信会被<b>拦截</b>，其他短信正常转发。';$('cfListLabel').innerText='拦截的关键词 (屏蔽这些)';}
}
updCfMode();
...
function upd(i){var t=$('tp'+i).value;$('cf'+i).style.display=(t=='4')?'block':'none';var showK1=(t=='5'||t=='7');$('k1'+i).style.display=showK1?'block':'none';var lbl=$('k1l'+i);if(lbl){lbl.innerText=(t=='5')?'Chat ID':'加签密钥 (可选)';}}
)rawjs";

// JavaScript 代码 (Part 2: 自动加载和操作函数)
...
function autoLoad(){
  fetch('/stats').then(r=>r.json()).then(d=>{
    ...
    var h=Math.floor(d.uptime/3600);$('upT').innerText='运行 '+h+' 小时 / 内存 '+(d.freeHeap/1024).toFixed(0)+'K';
...
function act(t){
  ...
  if(t==='sms'){var p=$('sPh').value,x=$('sTx').value;if(!p||!x)return toast('请填写号码和内容');toast('发送中...');postJ('/sendsms',{phone:p,content:x},d=>toast(d.message));}
  if(t==='ping'){var l=$('pLog');l.style.display='block';l.innerText='正在 Ping 8.8.8.8 ...';postJ('/ping',{},d=>{l.innerText=d.message;l.style.color=d.success?'#15803d':'#b91c1c';});}
}
...
function toggleAirplane(){
  ...
  postJ('/airplane',{enabled:!isOn},d=>{
    if(d.success){sw.className='sw'+(d.enabled?' on':'');$('apBadge').className='badge '+(d.enabled?'b-warn':'b-ok');$('apBadge').innerText=d.enabled?'已开启':'已关闭';log.innerText=d.message;log.style.color=d.enabled?'#a16207':'#15803d';}
...
// JavaScript 代码 (Part 3: 短信历史和保存函数)
...
function loadHist(){
  ...
  fetch('/history').then(r=>r.json()).then(d=>{
    smsData=d.history||[];
    if(smsData.length==0){$('contactList').innerHTML='<div style="text-align:center;padding:30px 10px;color:#94a3b8;font-size:0.85em">暂无短信</div>';$('chatMessages').innerHTML='<div class="chat-empty"><div style="font-size:2em;margin-bottom:10px">📭</div><div>暂无短信记录</div></div>';return;}
    var contacts={};
  ...
  }).catch(e=>{console.error('加载短信失败',e);$('contactList').innerHTML='<div style="text-align:center;padding:20px;color:#b91c1c;font-size:0.85em">加载失败</div>';});
}
...
function selContact(name){
  ...
  if(msgs.length===0){$('chatMessages').innerHTML='<div class="chat-empty">无消息</div>';return;}
  ...
  $('chatCount').innerText=msgs.length+'条短信';$('chatCount').style.marginLeft='36px';
...
function saveFilter(){var ls=$('ftList').value.split(/[,，\n]/).map(s=>s.trim()).filter(s=>s).join(',');postJ('/filter',{enabled:$('ftEn').value==='true',whitelist:$('ftWl').value==='true',numbers:ls.split(',')},d=>toast('已保存号码过滤设置'));}
function saveContentFilter(){var ls=$('cfList').value.split(/[,，\n]/).map(s=>s.trim()).filter(s=>s).join(',');postJ('/contentfilter',{enabled:$('cfEn').value==='true',whitelist:$('cfWl').value==='true',keywords:ls},d=>toast('已保存关键词过滤设置'));}
function saveTimer(){postJ('/timer',{enabled:$('tmEn').value==='true',type:+$('tmType').value,interval:+$('tmInt').value,phone:$('tmPh').value,message:$('tmMsg').value,startDate:$('tmStart').value},d=>toast('已保存定时任务'));}

function saveAll(e){
  e.preventDefault();toast('正在保存配置...');
  ...
  .then(r=>r.json()).then(d=>{
    if(d.success){toast('配置已保存');if(d.needRestart&&confirm('配置已保存成功！\n\n是否立即重启设备使配置生效？')){toast('设备重启中...');postJ('/restart',{},()=>{});setTimeout(()=>location.reload(),3000);}}
